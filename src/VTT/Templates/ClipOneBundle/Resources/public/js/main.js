...
var idMenuPrincipal = 'listaCadastro';
var base = $("#base-url-sistema").text();
//Main Function
var Main = function() {
    ...
    */
    ...
    var runActiveMenu = function(mnuMemory) {

        if (mnuMemory === true) {
            return true;
        }
        base = $.trim(base);
        var url = window.document.URL.replace(base, '');
        url = url.split('/');
        var itemUrl = $.trim(url[0]);
        var itemUrlOpcional = $.trim(url[1]);

        var $listaCadastroPrincipal = $("#listaCadastro");
        var $listaOperacao = $("#listaOperacoes");

        
        deselectedMnu(
                [
                    $listaCadastroPrincipal,
                    $listaOperacao
                ]
                );


        var subItensCadastroPrincial = [
            'administradoras',
            'empresas',
            'clientes',
            'planos',
        ...
        var subItensOperacao = [
            'vendas',
            'planosgerenciar-planos-clientes',
            'relatorios-administradora',
            'cartoes',
            'vendas',
            'movimentacao-produtos-troca',
            'relatorios-empresa'
        ];

        if ($.inArray(itemUrl, subItensCadastroPrincial) > -1 ) {
            selectedMnu($listaCadastroPrincipal, $("#mnu-" + itemUrl));
        } 
        if ($.inArray(itemUrl, subItensOperacao) > -1) {
            menuPrincipal = false;
            selectedMnu($listaOperacao, $("#mnu-" + itemUrl));
        } 
       
        if ($.inArray(itemUrl + itemUrlOpcional, subItensOperacao) > -1) {
            menuPrincipal = false;
            selectedMnu($listaOperacao, $("#mnu-" + itemUrl + itemUrlOpcional));
        } 
        
       
        base = base + url[0] + '/' + url[1];
        
        if (base === window.document.URL && menuPrincipal === true) {
            if (!$listaCadastroPrincipal.hasClass('open'))
                $listaCadastroPrincipal.addClass('open');

            if (!$listaCadastroPrincipal.hasClass('active'))
                $listaCadastroPrincipal.addClass('active');
        }

    };

    var deselectedMnu = function(lists) {
        $.each(lists, function(i, item) {
            item.removeClass('open active');
            item.find('.sub-menu li').removeClass('open active');
            item.find('.sub-menu li .sub-menu li').removeClass('open active');
        });
    };

    var selectedMnu = function(listaPrincipal, mnuId) {
        if($("body").find(mnuId).size() == 0 )return true;
        listaPrincipal.closest('ul').find('li').removeClass('open active');
        listaPrincipal.addClass("open active");
        mnuId.addClass('open active');
        var li = mnuId.closest('ul').closest('li');
        if (li.attr('id') !== listaPrincipal.attr('id')) {
    ...
    var runNavigationMenu = function() {
        ...
        $('.main-navigation-menu > li a').bind('click', function() {
            ...
            var liPai = $(this).closest('li');

            if ($(this).closest('li').prop('id') !== idMenuPrincipal) {
                menuPrincipal = false;
            } else {
                menuPrincipal = true;
            }

            if (liPai.children('ul').hasClass('sub-menu') && (!$('body').hasClass('navigation-small')) || !ul.hasClass('main-navigation-menu')) {
                if (!liPai.hasClass('open')) {
                    liPai.addClass('open');
                    ul.children('li.open').not(liPai).removeClass('open').children('ul').slideUp(200);
                    liPai.children('ul').slideDown(200, function() {
                        runContainerHeight();
                    });
                } else {
                    ul.children('li.open').removeClass('open').children('ul').slideUp(200, function() {
                        runContainerHeight();
                    });
                }
            }
